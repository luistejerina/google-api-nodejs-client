...
describe('Clients', () => {
    ...
    before((done) => {
        ...
        async.parallel([
            (cb) => {
                utils_1.default.loadApi(google, 'plus', 'v1', {}, cb);
            },
            (cb) => {
                utils_1.default.loadApi(google, 'oauth2', 'v2', {}, cb);
            }
        ], (err, apis) => {
            if (err) {
                return done(err);
    ...
    it('should support default params', (done) => {
        ...
        const req = datastore.projects.lookup({ projectId: 'test-project-id' }, utils_1.default.noop);
        ...
        nock.enableNetConnect();
        utils_1.default.loadApi(google, 'datastore', 'v1beta3', {
            params: { myParam: '123' }
        }, (err, datastore) => {
            nock.disableNetConnect();
            if (err) {
    ...
    it('should include default params when only callback is provided to API call', (done) => {
        ...
        const req = datastore.projects.lookup(utils_1.default.noop);
