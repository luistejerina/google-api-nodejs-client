...
function testMultpart(drive, cb) {
    ...
    let expectedResp = fs.readFileSync(path.join(__dirname, '/fixtures/media-response.txt'), { encoding: 'utf8' });
...
describe('Media', () => {
    ...
    it('should not multipart upload if no media body given', () => {
        let req = localDrive.files.insert({ someAttr: 'someValue' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'someAttr=someValue');
        req = remoteDrive.files.insert({ someAttr: 'someValue' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'someAttr=someValue');
    });
    ...
    it('should handle metadata-only media requests properly', (done) => {
        ...
        let req = localGmail.users.drafts.create({ userId: 'me', resource: resource, media: { mimeType: 'message/rfc822' } }, (err, resp) => {
            if (err) {
                return done(err);
            }
            assert.equal(req.headers['content-type'], 'application/json');
            assert.equal(JSON.stringify(resp), JSON.stringify(resource));
            req = remoteGmail.users.drafts.create({ userId: 'me', resource: resource, media: { mimeType: 'message/rfc822' } }, (err, resp) => {
                if (err) {
                    return done(err);
                }
                assert.equal(req.headers['content-type'], 'application/json');
    ...
    it('should accept readable stream as media body with metadata', (done) => {
        ...
        let expectedBody = fs.readFileSync(path.join(__dirname, '/fixtures/media-response.txt'), { encoding: 'utf8' });
        let req = localGmail.users.drafts.create({
            ...
            expectedBody = fs.readFileSync(path.join(__dirname, '/fixtures/media-response.txt'), { encoding: 'utf8' });
