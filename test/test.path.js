...
describe('Path params', () => {
    ...
    it('should not throw error if not included and required', (done) => {
        assert.doesNotThrow(() => {
            localDrive.files.get({}, utils_1.default.noop);
            remoteDrive.files.get({}, utils_1.default.noop);
            done();
        });
    });
    ...
    it('should return null request object if not included and required', () => {
        let req = localDrive.files.get({}, utils_1.default.noop);
        assert.equal(req, null);
        req = remoteDrive.files.get({}, utils_1.default.noop);
        assert.equal(req, null);
    });
    ...
    it('should not be modifiable directly', () => {
        ...
        assert.doesNotThrow(() => {
            ...
            localDrive.files.get(options, utils_1.default.noop);
            localDrive.files.get(options, utils_1.default.noop);
            remoteDrive.files.get(options, utils_1.default.noop);
            remoteDrive.files.get(options, utils_1.default.noop);
        });
    });
    it('should be put in URL of path', () => {
        let req = localDrive.files.get({ fileId: 'abc123' }, utils_1.default.noop);
        assert.equal(req.uri.path, '/drive/v2/files/abc123');
        req = remoteDrive.files.get({ fileId: 'abc123' }, utils_1.default.noop);
        assert.equal(req.uri.path, '/drive/v2/files/abc123');
    });
    it('should be put in URL of pathname', () => {
        let req = localDrive.files.get({ fileId: '123abc' }, utils_1.default.noop);
        assert.equal(req.uri.pathname, '/drive/v2/files/123abc');
        req = remoteDrive.files.get({ fileId: '123abc' }, utils_1.default.noop);
        assert.equal(req.uri.pathname, '/drive/v2/files/123abc');
    });
    it('should not be urlencoded', () => {
        let req = localDrive.files.get({ fileId: 'p@ram' }, utils_1.default.noop);
        assert.equal(req.uri.path.split('/').pop(), 'p@ram');
        req = remoteDrive.files.get({ fileId: 'p@ram' }, utils_1.default.noop);
        assert.equal(req.uri.path.split('/').pop(), 'p@ram');
    });
    ...
    it('should keep query params as is', () => {
        let req = localDrive.files.get({ fileId: '123abc', hello: 'world' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'hello=world');
        req = remoteDrive.files.get({ fileId: '123abc', hello: 'world' }, utils_1.default.noop);
        assert.equal(req.uri.query, 'hello=world');
    });
    after(() => {
        nock.cleanAll();
